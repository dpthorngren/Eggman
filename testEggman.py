...
def testKepler():
    '''Tests that eggman.solve_kepler successfully solves Kepler's equation.'''
    # Known values
    for eccen in np.random.rand(100):
        assert eggman.solve_kepler(0, eccen) == approx(0., 1e-12)
        assert eggman.solve_kepler(np.pi, eccen) == approx(np.pi, 1e-12)

    # Check that other solutions actually solve the equation
    for ma in np.random.rand(100) * 50 - 10:
        assert eggman.solve_kepler(ma, 0) == approx(ma, 1e-12)
        for eccen in np.random.rand(100):
            if eccen == 1:
                continue
            ea = eggman.solve_kepler(ma, eccen)
...
def testOrbitPositions():
    ...
    for eccen, lon_periapse, inc in np.random.rand(100, 3):
        ...
        assert eggman.orbit_to_position(0., 15, 10, eccen, inc, lon_periapse)[0] == approx(0., abs=1e-10)
        # t=period/2 is mid-secondary-transit if lon_periapse=+/090, so x=0
        assert eggman.orbit_to_position(5.0, 15, 10, eccen, inc, 90.)[0] == approx(0., abs=1e-10)
        assert eggman.orbit_to_position(5.0, 15, 10, eccen, inc, -90.)[0] == approx(0., abs=1e-10)
    # Circular orbit checks
    assert eggman.orbit_to_position(0., 1., 4., 0.0, 90., 0.) == approx([0., 0., 1.], abs=1e-10)
    assert eggman.orbit_to_position(0.5, 1., 4., 0.0, 90., 0.) == approx([np.sqrt(.5), 0., np.sqrt(.5)], abs=1e-10)
    for inc, t in np.random.rand(100, 2) * np.pi:
        theta = t * 2 * np.pi / 4.
        pos = [np.sin(theta), np.cos(inc) * np.cos(theta), np.sin(inc) * np.cos(theta)]
        assert eggman.orbit_to_position(t, 1., 4., 0.0, inc * 180 / np.pi, 0.) == approx(pos, abs=1e-10)
    # Checking the signs of the output for correct orientation
    pos = eggman.orbit_to_position(0., 13.5, 2.3, 0.53, 82., 13.)
    assert pos[0] == approx(0, abs=1e-10)
    assert pos[1] > 0
    assert pos[2] > 0
    pos = eggman.orbit_to_position(.13, 13.5, 2.3, 0.53, 105., 12.)
    assert pos[0] > 0
    assert pos[1] < 0
    assert pos[2] > 0
    pos = eggman.orbit_to_position(1.17, 13.5, 2.3, 0.53, 85., 95.)
    assert pos[0] < 0
    assert pos[1] < 0
    assert pos[2] < 0
    ...
    pos = eggman.orbit_to_position(t_peri, 10., 5.3, 0.3, 87., 45.)
    assert np.linalg.norm(pos) == approx(10. * (1.-.3), 1e-9)
    t_apo = 5.3/2. - 0.3569835522205464
    pos = eggman.orbit_to_position(t_apo, 10., 5.3, 0.3, 87., 45.)
    assert np.linalg.norm(pos) == approx(10. * (1.+.3), 1e-9)
    # Distance on-sky from star
    # eggman.orbit_to_position args: t, semimajor, period, eccen, inclination, lon_periapse
    # batman.rsky args: t, t_conj, period, semimajor, inclination, eccen, lon_periapse, transittype, nthreads
    pos = eggman.orbit_to_position(2.1, 15., 5., .3, 89., 35.)
    ...
    pos = eggman.orbit_to_position(-1.2, 13., 4., .3, 89., 63.)
    ...
    pos = eggman.orbit_to_position(-1.2, 13., 4., .3, 85., 63.)
