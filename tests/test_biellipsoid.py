...
def test_biellipsoid_rotation():
    ...
    bell = BiellipsoidWrap(1., 1., 1., 1., phi=25.)
    ...
    bell = BiellipsoidWrap(1., 1., 1., 1., gamma=35.)
    ...
    for theta, phi, gamma in 179. * np.random.rand(100, 3) - 90.:
        bell = BiellipsoidWrap(1., 1., 1., 1., theta=theta, phi=phi, gamma=gamma)
    ...
    bell = BiellipsoidWrap(3., 2., 1., 1.5, theta=30., phi=10., x=3., y=5., z=-2.)
...
def test_biellipsoid_orbit_rotation():
    ...
    for angles in 179. * np.random.rand(100, 3) - 90.:
        ...
        bell = BiellipsoidWrap(1., 1., 1., 1., *angles, ci, *pos)
        assert bell.rot.T @ bell.rot == approx(np.eye(3))
    # Check that rotations go in the right direction
    bell = BiellipsoidWrap(1., 1., 1., 2., x=1.5, ci=0)
    assert bell.x_bounds() == approx([-0.5, 3.5])
    assert bell.y_bounds() == approx([-1, 1])
    pos = [-1.5, 0., 0.]
    bell = BiellipsoidWrap(1., 1., 1., 2., x=-1.5, ci=1)
    ...
    assert bell.x_bounds() == approx([-0.5, 3.5])
    assert bell.y_bounds() == approx([-1, 1])
    bell = BiellipsoidWrap(1., 1., 1., 2., ci=0., z=3, phi=180)
    assert bell.rot == approx(np.diag([-1, 1, -1]))
    # Verify no rotation at orbital origin (0, 0, 1)
    bell = BiellipsoidWrap(1., 1., 1., 2., z=2, ci=-2)
    assert bell.rot == approx(np.eye(3))
    bell = BiellipsoidWrap(1., 1., 1., 2., z=2, ci=0.)
    assert bell.rot == approx(np.eye(3))
    # Compare with rotation matrix helper
    bell = BiellipsoidWrap(3., 2., 1., 1.5, ci=0., x=3., y=0., z=3., theta=30, phi=10)
    ...
    for pos in np.random.rand(100, 3) * 2 - 1:
        ...
        ci = _decompose_position_(*pos)[3]
        bell = BiellipsoidWrap(1., 1., 1., 1., ci=ci, x=pos[0], y=pos[1], z=pos[2])
...
def test_biellipsoid_bounds():
    ...
    bell = BiellipsoidWrap(1., 2., 3., 4.)
    assert bell.x_bounds() == approx([-2, 1])
    assert bell.y_bounds() == approx([-3, 3])
    bell = BiellipsoidWrap(1., 2., 1., 1., theta=15.)
...
def test_transforms():
    for i in range(20):
        ...
        bell = BiellipsoidWrap(*radii, *angles, -2, *position)
...
def test_biellipsoid_point_visible():
    bell = BiellipsoidWrap(1.33, 1.33, 1.33, 1.33, theta=30, phi=1.5)
    ...
    bell = BiellipsoidWrap(.15, .1, .1, .1, x=5., y=3., theta=30., phi=1.5)
...
def test_biellipsoid_intersect():
    bell = BiellipsoidWrap(1.33, 1.33, 1.33, 1.33)
    ...
    bell = BiellipsoidWrap(1.2, 0.5, 1.2, 1.2)
    ...
    for i in range(100):
        ...
        radii = 1.5 * np.random.rand(4) + .5
        bell = BiellipsoidWrap(*radii, *angles, -2, *loc)
