...
def test_kepler():
    ...
    for eccen in np.random.rand(100):
        assert OrbitWrap.solve_kepler(0, eccen) == approx(0., 1e-12)
        assert OrbitWrap.solve_kepler(np.pi, eccen) == approx(np.pi, 1e-12)

    # Check that other solutions actually solve the equation
    for ma in np.random.rand(100) * 50 - 10:
        assert OrbitWrap.solve_kepler(ma, 0) == approx(ma, 1e-12)
        for eccen in np.random.rand(1000):
            if eccen == 1:
                continue
            ea = OrbitWrap.solve_kepler(ma, eccen)
...
def test_signs():
    ...
    orb = OrbitWrap(2.3, 0.0, 13.5, 0.53, 82., 13.)
    pos = orb.get_position(0.)
    assert pos[0] == approx(0, abs=1e-10)
    assert pos[1] > 0
    assert pos[2] > 0
    orb = OrbitWrap(2.3, 0.0, 13.5, 0.53, 105., 12.)
    pos = orb.get_position(.13)
    assert pos[0] > 0
    assert pos[1] < 0
    assert pos[2] > 0
    orb = OrbitWrap(2.3, 0.0, 13.5, 0.53, 85., 95.)
...
def test_orbit_positions():
    ...
    for inc, t in np.random.rand(100, 2) * np.pi:
        ...
        orb = OrbitWrap(4., 0., 1., 0.0, inc * 180 / np.pi, 0.)
...
def test_numerical():
    ...
    orb = OrbitWrap(5.3, 0., 10., 0.3, 87., 45.)
