...
def test_t3d_grazing():
    ...
    assert eggman.transit3d(.1 + 1e-3, .11, .1, .1, **dict(baseArgs)) < 1.
    assert eggman.transit3d(.1 + 1e-9, .1, .09, .09, **dict(baseArgs)) < 1.

    # Check for errors on barely non-grazing trants
    assert eggman.transit3d(.1 - 1e-3, .11, .1, .1, **dict(baseArgs)) == 1.
    assert eggman.transit3d(.1 - 1e-12, .1, .09, .09, **dict(baseArgs)) == 1.

    # Grazing on egress
    baseArgs['t'] = -baseArgs['t']
    assert eggman.transit3d(.11, .1 + 1e-3, .1, .1, **dict(baseArgs)) < 1.
    assert eggman.transit3d(.1, .1 + 1e-9, .09, .09, **dict(baseArgs)) < 1.

    # Grazing on top (true grazing transit)
    baseArgs['inclination'] = 90. - abs(baseArgs['t'][0]) * 180 / np.pi
    baseArgs['t'] = np.array([0.])
    assert eggman.transit3d(.11, .1, .1 + 1e-3, .1, **dict(baseArgs)) < 1.
    # TODO: Would like to pass these, but they don't actually violate my <1 PPM precision target.
    # assert eggman.transit3d(.09, .1, .1 + 1e-6, .1, **dict(baseArgs)) < 1.
    # assert eggman.transit3d(.1, .09, .1 + 1e-6, .1, **dict(baseArgs)) < 1.

    # Barely non-grazing on top (true grazing transit)
    assert eggman.transit3d(.11, .1, .1 - 1e-3, .1, **dict(baseArgs)) == 1.
    assert eggman.transit3d(.09, .1, .1 - 1e-6, .1, **dict(baseArgs)) == 1.
    assert eggman.transit3d(.1, .09, .1 - 1e-12, .1, **dict(baseArgs)) == 1.


def test_t3d_is_transiting():
    ...
    # Non-transiting
    assert eggman.transit3d(.1, .1, .1, .1, **dict(baseArgs, t=np.array([5.]))) == 1.
    assert eggman.transit3d(.1, .1, .1, .1, **dict(baseArgs, t=np.array([3.]))) == 1.
    assert eggman.transit3d(
        1.1, 1.1, 1.1, 1.1, **dict(baseArgs, t=np.array([2.51]), semimajor=1.)) == 1.
    assert eggman.transit3d(.1, .1, .1, .1, **dict(baseArgs, t=np.array([-25.01]))) == 1.
    # Transiting
    assert eggman.transit3d(
        1.1, 1.1, 1.1, 1.1, **dict(baseArgs, t=np.array([0.0]), semimajor=1.)) < 1.
    assert eggman.transit3d(
        1.1, 1.1, 1.1, 1.1, **dict(baseArgs, t=np.array([2.49]), semimajor=1.)) < 1.
    assert eggman.transit3d(.01, .015, .011, .011, **dict(baseArgs, t=np.array([.01]))) < 1.
    assert eggman.transit3d(.01, .015, .011, .011, **dict(baseArgs, t=np.array([-10.01]))) < 1.
    results = eggman.transit3d(
...
def test_t3d_analytic():
    ...
    assert eggman.transit3d(.1, .1, .1, .1, **baseArgs) == approx(1 - .1*.1, abs=1e-7)
    assert eggman.transit3d(.12, .19, .15, .15, **baseArgs) == approx(
        1 - (.12*.15 + .19*.15) / 2., abs=1e-7)
    assert eggman.transit3d(.01, .01, .02, .02, **baseArgs) == approx(
