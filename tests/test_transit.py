...
def test_transit_grazing():
    ...
    assert eggman.asymmetricTransit(.1 + 1e-3, .09, -1, **dict(baseArgs)) < 1.
    ...
    assert eggman.asymmetricTransit(.1 + 1e-3, .1 - 1e-3, -1., **dict(baseArgs)) < 1.
...
def test_transit_is_transiting():
    ...
    assert eggman.asymmetricTransit(
        1.1, 1.1, 1.1, **dict(baseArgs, t=np.array([2.51]), semimajor=1.)) == 1.
    assert eggman.asymmetricTransit(.1, .1, .1, **dict(baseArgs, t=np.array([-25.01]))) == 1.
    # Transiting
    assert eggman.asymmetricTransit(
        1.1, 1.1, 1.1, **dict(baseArgs, t=np.array([0.0]), semimajor=1.)) < 1.
    assert eggman.asymmetricTransit(
        1.1, 1.1, 1.1, **dict(baseArgs, t=np.array([2.49]), semimajor=1.)) < 1.
    ...
    results = eggman.asymmetricTransit(
        .1, .1, .1, **dict(baseArgs, t=np.linspace(-10, 10, 1000), inclination=80.))
    assert results == approx(1, abs=1e-12)
    # Would be non-transiting, but for the eccentricity
    assert eggman.asymmetricTransit(
        1.1, 1.1, 1.1,
        **dict(baseArgs, t=np.array([2.51]), eccen=0.1, lonPeriapse=-90, semimajor=1.)) < 1.
...
def test_transit_analytic():
    ...
    assert eggman.asymmetricTransit(.1, .2, -1, **baseArgs) == approx(
        1 - (.1*.1 + .2*.2) / 2., abs=1e-7)
    assert eggman.asymmetricTransit(.12, .19, .15, **baseArgs) == approx(
        1 - (.12*.15 + .19*.15) / 2., abs=1e-7)
    assert eggman.asymmetricTransit(.01, .01, .02, **baseArgs) == approx(
...
def test_transit_numerical():
    ...
    result = eggman.asymmetricTransit(.12, .12, .12, **baseArgs)
    ...
    result = eggman.asymmetricTransit(.11, .09, -1, **baseArgs)
